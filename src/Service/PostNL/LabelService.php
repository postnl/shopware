...
class LabelService
{
    ...
    {
        //Collect all labels and sort them
        $labels = [];
        foreach ($responseShipments as $responseShipment) {
            foreach ($responseShipment->getResponseShipments()[0]->getLabels() as $label) {
                $labels[] = $label;
        ...
        //Start a new pdf
        // Disable header and footer
        $pdf = new RFPdi(self::ORIENTATION_FORMAT_LANDSCAPE, 'mm', $mergedFormat === self::LABEL_FORMAT_A4 ? [210, 297] : [105, 148]);
        ...
        foreach ($labels as $label) {
            ...
            if ($sizeAndOrientation['iso'] == $mergedFormat) {
                ...
                // Add a config if we do not have any
